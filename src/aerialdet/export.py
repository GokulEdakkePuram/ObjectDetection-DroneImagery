...
ONNX covers cross-platform serving; CoreML is the sensible target when the
model has to run on Apple hardware.
"""
...
def export(weights: str, fmt: str = "onnx", imgsz: int = 960, half: bool = False) -> Path:
    ...
    model = YOLO(weights)
    out = model.export(format=fmt, imgsz=imgsz, half=half)
    return Path(out)
