...
`finetune_960` run on a laptop, a rented 4090 and a future local GPU without
editing the experiment -- and it is what makes a constant batch size across a
resolution sweep possible, which removes the biggest confound from the
ablation.
"""
...
def detect() -> Hardware:
    ...
    if torch.backends.mps.is_available():
        ...
        return Hardware(
            "mps", "Apple Silicon (unified memory)", psutil.virtual_memory().total / 1024**3
        )

    import psutil

    return Hardware("cpu", "CPU", psutil.virtual_memory().total / 1024**3)


def auto_profile(hw: Hardware | None = None) -> str:
    """Pick the profile that matches the detected hardware.

    Thresholds are deliberately conservative: a profile that OOMs three hours
